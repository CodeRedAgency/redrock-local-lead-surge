...
type StandardCleaningChecklistSectionProps = {
  title?: string;
  intro?: string;
...
export function StandardCleaningChecklistSection({
  title,
  intro,
  sectionClassName,
  containerClassName
}: StandardCleaningChecklistSectionProps) {
  const { t } = useTranslation();

  const resolvedTitle = title ?? t("checklists.standardCleaning.title");
  const resolvedIntro = intro ?? t("checklists.standardCleaning.intro");
  const notIncludedTitle = t("checklists.standardCleaning.notIncludedTitle");
  const notIncludedIntro = t("checklists.standardCleaning.notIncludedIntro");
  const notIncludedItems = t("checklists.standardCleaning.notIncludedItems", { returnObjects: true }) as string[];
  const note = t("checklists.standardCleaning.note");
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid gap-8 md:grid-cols-2">
            {SECTION_CONFIG.map(({ key, icon }) => {
              const sectionTitle = t(`checklists.standardCleaning.sections.${key}.title`);
              const sectionItems = t(`checklists.standardCleaning.sections.${key}.items`, {
                returnObjects: true
              }) as string[];
...
export default StandardCleaningChecklistSection;
